...
class GFWicketDataHiddenField extends GF_Field
{
    ...
    public static function editor_script(): void
    {
        ...
        <script type="text/javascript">
            ...
            function toggleWicketLiveUpdateSettings(enabled, checkboxElement = null) {
                ...
                console.log('GFWicketDataHiddenField: toggleWicketLiveUpdateSettings context:', $context.length ? $context.get(0) : 'Not Found', 'Enabled:', enabled);
                $context.find('.wicket-gf-dependent-setting').css('display', enabled ? '' : 'none');
            }

            jQuery(document).on('gform_load_field_settings', function(event, field, form) {
                console.log('GFWicketDataHiddenField: gform_load_field_settings event. Field Type: ' + field.type, 'Field Object:', field);

                if (field.type === 'wicket_data_hidden') {
                    console.log('GFWicketDataHiddenField: Processing settings for wicket_data_hidden.');
                    ...
                    if (!$fieldSpecificSettingsArea.length) {
                        ...
                        $fieldSpecificSettingsArea = jQuery('#field_settings_tab_container .gform_editor_panel_content');
                         console.log('GFWicketDataHiddenField: Using fallback settings area:', $fieldSpecificSettingsArea.length ? $fieldSpecificSettingsArea.get(0) : 'Fallback not found');
                    }
                    if (!$fieldSpecificSettingsArea.length) {
                        $fieldSpecificSettingsArea = jQuery(document); // Absolute fallback
                        console.warn('GFWicketDataHiddenField: Could not find specific settings area, using document scope. This might lead to issues if multiple such fields exist.');
                    }

                    var $checkbox = $fieldSpecificSettingsArea.find('#liveUpdateEnabled');
                    console.log('GFWicketDataHiddenField: Checkbox #liveUpdateEnabled found in scope:', $checkbox.length ? $checkbox.get(0) : 'Not found in scope');

                    var $liveUpdateEnableLi = $checkbox.closest('li.live_update_enable_setting');
                    console.log('GFWicketDataHiddenField: li.live_update_enable_setting (closest to checkbox):', $liveUpdateEnableLi.length ? $liveUpdateEnableLi.get(0) : 'Not found');

                    if ($liveUpdateEnableLi.length) {
                        console.log('GFWicketDataHiddenField: Attempting to show li.live_update_enable_setting. Current style:', $liveUpdateEnableLi.attr('style'));
                        ...
                        console.log('GFWicketDataHiddenField: li.live_update_enable_setting style after .css("display", "") and .show():', $liveUpdateEnableLi.attr('style'));
                    } else {
                        console.error('GFWicketDataHiddenField: CRITICAL - Could not find li.live_update_enable_setting to make it visible.');
                    }

                    if ($checkbox.length) {
                        $checkbox.prop('checked', field.liveUpdateEnabled || false);
                        console.log('GFWicketDataHiddenField: Checkbox #liveUpdateEnabled checked state set to:', field.liveUpdateEnabled || false);
                        ...
                        $settingsScope.find('#liveUpdateSchemaKey').val(field.liveUpdateSchemaKey || '');
                        $settingsScope.find('#liveUpdateValueKey').val(field.liveUpdateValueKey || '');
                        console.log('GFWicketDataHiddenField: Dependent field values set.');

                        var liveUpdateCheckboxElement = $checkbox.get(0);
                        if (typeof toggleWicketLiveUpdateSettings === 'function') {
                            console.log('GFWicketDataHiddenField: Calling toggleWicketLiveUpdateSettings.');
                            toggleWicketLiveUpdateSettings(field.liveUpdateEnabled || false, liveUpdateCheckboxElement);
                        } else {
                            console.error('GFWicketDataHiddenField: toggleWicketLiveUpdateSettings function is undefined!');
                        }
                    } else {
                        console.error('GFWicketDataHiddenField: Checkbox #liveUpdateEnabled not found. Cannot set initial values or toggle dependents.');
                    }
                    console.log('GFWicketDataHiddenField: Finished processing settings for wicket_data_hidden.');
                }
            });
        </script>
        <?php
    ...
    public function get_field_input($form, $value = '', $entry = null): string
    {
        ...
        $id = (int) $this->id;
        $field_id = "input_{{$form['id']}_{$id}";
        $input_value = esc_attr($value);
        $data_attributes = '';
        if (!empty($this->liveUpdateEnabled)) {
            $data_attributes .= ' class="wicket-gf-live-update-target"';
        ...
        return sprintf("<input name='input_%d' id='%s' type='hidden' value='%s'%s />", $id, $field_id, $input_value, $data_attributes);
    }
}

GF_Fields::register(new GFWicketDataHiddenField());
