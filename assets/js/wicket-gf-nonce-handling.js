...
jQuery(document).ready(function($) {
    ...
    $(document).on(
        "change",
        '[id^="input_"][id$="_org_search_select"]',
        function () {
            var $field = $(this);
            ...
            if ($field.val()) {
                sessionStorage.setItem(
                    "wicket_gf_org_selection_" + fieldId,
                    $field.val()
                );
                sessionStorage.setItem(
                    "wicket_gf_org_selection_text_" + fieldId,
    ...
    $('[id^="input_"][id$="_org_search_select"]').each(function () {
        var $field = $(this);
        var fieldId = $field.attr("id");
        var savedValue = sessionStorage.getItem(
            "wicket_gf_org_selection_" + fieldId
        );
        ...
        $(window)
            ...
            .on("orgss-selection-made", function (event) {
                ...
                setTimeout(function () {
                    ...
                    var $activeMembershipModal = $(
                        ".component-org-search-select__active-membership-alert"
                    );
                    var $confirmationPopup = $(
                        ".component-org-search-select__confirmation-popup"
                    );
                    ...
                    if ($activeMembershipModal.length > 0) {
                        var isActiveMembershipVisible =
                            $activeMembershipModal.is(":visible") &&
                    ...
                    if (!hasModal && $confirmationPopup.length > 0) {
                        var isConfirmationVisible =
                            $confirmationPopup.is(":visible") &&
                    ...
                    if (hasModal) {
                        ...
                        var modalCheckInterval = setInterval(function () {
                            ...
                            if ($activeMembershipModal.length > 0) {
                                modalStillVisible =
                                    $activeMembershipModal.is(":visible") &&
                                    $activeMembershipModal.css("opacity") !==
                                        "0" &&
                                    $activeMembershipModal.css(
                                        "pointer-events"
                                    ) !== "none";
                            }
                            ...
                            if (
                                !modalStillVisible &&
                                $confirmationPopup.length > 0
                            ) {
                                modalStillVisible =
                                    $confirmationPopup.is(":visible") &&
                                    $confirmationPopup.css("opacity") !== "0" &&
                                    $confirmationPopup.css("pointer-events") !==
                                        "none";
                            }
                            ...
                            if (!modalStillVisible) {
                                ...
                                $(
                                    '[name^="input_"][name*="_org_search"], [name^="input_"][value!=""]'
                                ).each(function () {
                                    if (
                                        $(this).val() &&
                                        $(this).val().length > 0
