...
add_filter('gform_validation', function ($validation_result) {
    ...
    if (is_array($form) && isset($form['fields'])) {
        foreach ($form['fields'] as $field) {
            ...
            if ($field_type === 'wicket_widget_ai') {
                Wicket()->log()->debug('Found wicket_widget_ai field ' . $field_id . ' in form ' . $form_id, ['source' => 'gravityforms-state-debug']);
            }
        }
    }
    ...
    if (!isset($_POST[$state_key])) {
        Wicket()->log()->debug('gform_validation: state missing for form ' . $form_id . ' POST keys: ' . json_encode(array_keys($_POST)), ['source' => 'gravityforms-state-debug']);
    ...
    if (!$decoded || !is_array($decoded) || count($decoded) !== 2) {
        Wicket()->log()->debug('gform_validation: invalid state payload for form ' . $form_id . ' raw_trunc: ' . substr($raw, 0, 1000) . ' decoded: ' . var_export($decoded, true), ['source' => 'gravityforms-state-debug']);
    ...
    if ($checksum !== $decoded[1]) {
        Wicket()->log()->debug('gform_validation: checksum mismatch for form ' . $form_id . ' computed: ' . $checksum . ' state_checksum: ' . var_export($decoded[1], true) . ' state0_trunc: ' . substr($decoded[0], 0, 1000), ['source' => 'gravityforms-state-debug']);

        return $validation_result;
    }

    $gf_state = json_decode($decoded[0], true);
    Wicket()->log()->debug('gform_validation: decoded gf_state for form ' . $form_id . ' gf_state: ' . var_export($gf_state, true) . ' relevant_post: ' . var_export(array_filter($_POST, function ($k) use ($form_id) { return strpos($k, 'input_') === 0 || strpos($k, 'gform_target_page_number_') === 0 || strpos($k, 'wicket_ai_info_data_') === 0; }, ARRAY_FILTER_USE_KEY), true), ['source' => 'gravityforms-state-debug']);
            ...
            if (isset($gf_state[$field_id])) {
                ...
                if (is_array($gf_state[$field_id])) {
                    if (!in_array($hash, $gf_state[$field_id], true)) {
                        ...
                        Wicket()->log()->debug('gform_validation: appended hash for form ' . $form_id . ' field ' . $field_id . ' hash=' . $hash, ['source' => 'gravityforms-state-debug']);
                        $modified = true;
                    }
                } else {
                    ...
                    if ($gf_state[$field_id] !== $hash) {
                        ...
                        Wicket()->log()->debug('gform_validation: converted state to array and added hash for form ' . $form_id . ' field ' . $field_id . ' hash=' . $hash, ['source' => 'gravityforms-state-debug']);
                        $modified = true;
                    }
                }
            } else {
                ...
                $gf_state[$field_id] = [$hash];
                Wicket()->log()->debug('gform_validation: created state and added hash for form ' . $form_id . ' field ' . $field_id . ' hash=' . $hash, ['source' => 'gravityforms-state-debug']);
                $modified = true;
            }
        }
    }
    ...
    if ($modified) {
        ...
        Wicket()->log()->debug('gform_validation: updated POST[' . $state_key . '] for form ' . $form_id . ' with new checksum', ['source' => 'gravityforms-state-debug']);
    }
    ...
    if (is_array($form) && isset($form['fields']) && is_array($form['fields'])) {
        foreach ($form['fields'] as $fld) {
            ...
            if ($fid === $target_field_id) {
                Wicket()->log()->debug('gform_validation: form ' . $form_id . ' contains field ' . $target_field_id . ' definition: ' . var_export($fld, true), ['source' => 'gravityforms-state-debug']);
                break;
            }
        }
    }
    ...
    Wicket()->log()->debug('gform_validation: form ' . $form_id . ' pages: current=' . var_export($current_page, true) . ' target=' . var_export($target_page, true) . ' POST keys count=' . count($_POST), ['source' => 'gravityforms-state-debug']);
